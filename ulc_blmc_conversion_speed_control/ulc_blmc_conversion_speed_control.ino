...
volatile unsigned long rx_pwm =0;
volatile unsigned char rotor_state = 0;
volatile unsigned char rotor_run = 0;
volatile unsigned char stopTiming=0;

volatile unsigned long current_time,timer_1,prev_time,tmp_pwm=0,prev_pwm=0;
volatile byte last_channel_1;
volatile int counter;
//volatile unsigned char startupFlag=0;
volatile unsigned char no_t1_IE=0;
...
ISR(PCINT2_vect){
  ...
  if(PIND & B00000001){                                        //Is input 8 high?
    ...
    if(current_time<timer_1) current_time =  current_time + 65535/2  ;
   ...
   if(tmp_pwm<prev_pwm-5|| tmp_pwm>prev_pwm+5){
   
  
      prev_pwm=tmp_pwm;

      if(prev_pwm<=10)prev_pwm=0;
      if(prev_pwm>=990)prev_pwm=1000;
