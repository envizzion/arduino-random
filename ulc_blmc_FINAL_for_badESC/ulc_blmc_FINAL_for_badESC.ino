...
//############################################################################
{
  if(rotor_run == 5) next_commutate_state (0);
...
ISR(PCINT2_vect){
  ...
  rx_timeOut=0;
  if(rotor_run>=5 || rx_pwm < START_PWM){
  ...
  if(PIND & B00000001){                                        //Is input 8 high?
    ...
    if((current_time - timer_1)>2500){TCNT1=0;counter=0;return;} 
    ...
    if(counter==5){
      tmp_pwm=tmp_pwm/20;
      if((tmp_pwm<rx_pwm- 1|| tmp_pwm>rx_pwm + 1) && tmp_pwm<255 ){
